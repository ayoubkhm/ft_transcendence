...
async function startGame(mode: GameMode, difficulty?: string) {
  ...
  try {
    ...
    const body = { mode, difficulty, isCustomOn: customToggle!.checked, userId };
    ...
    if (!res.ok) {
      const errorData = await res.json().catch(() => ({ error: 'Failed to start game and could not parse error response' }));
